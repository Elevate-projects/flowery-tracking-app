...
void main() {
  ...
  setUp(() {
    ...
    cubit = ProfileCubit(mockUseCase, mockLogoutUseCase, mockSecureStorage);
  ...
  group('ProfileCubit Tests', () {
    test("Initial state is correct", () {
      ...
      expect(
        cubit.state.profileStatus,
        const StateStatus<DriverDataEntity?>.initial(),
      );
    });

    test(
      "ProfileInitializationIntent sets language and fetches profile data",
      () async {
        final driver = DriverDataEntity(
          firstName: "Peter",
        ...
        await cubit.doIntent(
          ProfileInitializationIntent(globalCubit: mockGlobalCubit),
        );
    ...
    test("GetUserProfileDataIntent sets profileStatus to Success", () async {
      ...
      expect(
        cubit.state.profileStatus,
        isA<StateStatus>().having((s) => s.data, "driver", driver),
      );
      verify(mockUseCase.call()).called(1);
    });

    test(
      "GetUserProfileDataIntent sets profileStatus to Failure when use case fails",
      () async {
        final responseException = const ResponseException(message: "Failed");
        when(mockUseCase.call()).thenAnswer(
          (_) async => Failure(responseException: responseException),
        );
    ...
    test("ToggleLanguageIntent changes the selected language", () async {
      when(
        mockGlobalCubit.doIntent(intent: anyNamed('intent')),
      ).thenAnswer((_) async {});
    ...
    test(
      "ToggleLanguageIntent does not change language if same as current",
      () async {
        cubit.emit(cubit.state.copyWith(selectedLanguage: Languages.english));
        await cubit.doIntent(
          ToggleLanguageIntent(
